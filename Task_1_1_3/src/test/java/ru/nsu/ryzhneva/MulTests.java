...
public class MulTests {
    ...
    @Test
    void testEvaluation() {
        Expression exp = new Mul(x, five); // x * 5
    ...
    void testSimplificationWithOne() {
        // x * 1 simplifies to x
        Expression exp1 = new Mul(x, one);
        assertEquals("x", exp1.funcSimple().print());

        // 1 * x simplifies to x
        Expression exp2 = new Mul(one, x);
    ...
    void testSimplificationWithZero() {
        // x * 0 simplifies to 0
        Expression exp1 = new Mul(x, zero);
        assertEquals("0", exp1.funcSimple().print());

        // 0 * x simplifies to 0
        Expression exp2 = new Mul(zero, x);
